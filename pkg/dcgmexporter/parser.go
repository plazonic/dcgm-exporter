	...
	for i, record := range records {
		...
		if !ok && !oldOk {
			...
			if err != nil {
				...
				res.ExporterCounters = append(res.ExporterCounters, Counter{dcgm.Short(expField), record[0], record[1], record[2], alterField, alterHelp, multiplier})
				continue
			}
		}
		...
		if !useOld {
			...
			res.DCGMCounters = append(res.DCGMCounters, Counter{fieldID, record[0], record[1], record[2], alterField, alterHelp, multiplier})
		} else {
			...
			res.DCGMCounters = append(res.DCGMCounters, Counter{oldFieldID, record[0], record[1], record[2], alterField, alterHelp, multiplier})
		}
	}
